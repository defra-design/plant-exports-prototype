...
/* Chris Harding (04.01.22) - Commended out older cookie banner code until the alternative design is accepted by PHES */
...
exports.handleCookies = function (app) {
  return function handleCookies (req, res, next) {
    ...
    if (cookie === 'Accepted') {
      ...
      // Variables for the cookie banner
      app.locals.shouldShowCookieMessage = true
      app.locals.shouldShowCookieMessageStatus = "Accepted";

      // Variables for the cookies page
      app.locals.analytics = "Yes";
      ...
      // Variables for the cookie banner
      app.locals.shouldShowCookieMessage = true
      app.locals.shouldShowCookieMessageStatus = "Rejected";

      // Variables for the cookies page
      app.locals.analytics = "No";

      return next()
    }
    else if (cookie === 'Hidden') {

      // Variables for the cookie banner
      app.locals.shouldShowCookieMessage = false
      app.locals.shouldShowCookieMessageStatus = "Hidden";

      return next()
    }
    else {
      app.locals.shouldShowCookieMessage = true
      app.locals.shouldShowCookieMessageStatus = "Unset";
      next()
    }
